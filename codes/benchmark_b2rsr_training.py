...
def parse_cases(value):
    ...
    for item in value.split(','):
        ...
        if gt <= 0 or batch < 2 or gt % 4:
            raise argparse.ArgumentTypeError(
                'cases must use GT sizes divisible by 4 and batch >= 2, e.g. 192x8')
...
def benchmark_case(model, gt_size, batch_size, phase, warmup, steps, chunk_size):
    scale = 4
    lq_size = gt_size // scale
    lq = torch.rand(batch_size, 3, lq_size, lq_size)
    gt = torch.rand(batch_size, 3, gt_size, gt_size)
    step_base = 1 if phase == 'soft' else 25000
...
def main():
    ...
    parser.add_argument(
        ...
        default=parse_cases('96x16,96x32,128x16,192x8,192x12,192x16'),
        help='comma-separated GT-size x batch-size cases')
    ...
    print('\nphase={} warmup={} measured_steps={}'.format(args.phase, args.warmup, args.steps))
    print('GT   LR   batch  windows  step_ms  p90_ms  images/s  LR-Mpix/s  alloc_MiB  reserve_MiB')
    print('-' * 94)
    for gt_size, batch_size in args.cases:
        lq_size = gt_size // 4
        windows = (lq_size // 8) ** 2
        try:
            result = benchmark_case(
                model, gt_size, batch_size, args.phase,
