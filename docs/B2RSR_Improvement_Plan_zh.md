...
> B2R-SR 是一种面向高效图像超分辨率的、与主干网络无关的动态推理加速框架。它包装已有预训练超分模型，并在可控计算预算下，根据预测的重建收益为不同空间窗口自适应分配计算量。
...
> B2R-SR 并不是简单根据图像复杂度或高频强度分配计算，而是根据预测的边际重建收益进行动态计算分配。
...
主干网络仍然可以替换。B2R-SR 应该作为一个动态推理控制器，包装不同 SR 架构，而不是成为一个与特定 backbone 深度绑定的新网络。
...
这个模块必须保持轻量。它不能变成主要计算模块，否则 B2R-SR 的“加速框架”定位会被削弱。
...
> B2R-SR 为 CNN-based SR backbone 提供了一种通用、可控的动态推理插件，并在速度-质量权衡上优于已有区域感知加速方法。
...
> B2R-SR 是一个统一的收益感知计算分配框架，可同时适配 CNN 与 Transformer 超分主干，在多种退化条件下实现可控动态推理，并带来真实硬件加速。
